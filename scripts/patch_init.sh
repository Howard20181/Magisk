#####################################################################
#   Magisk in System Partition
#####################################################################
#
# Support API level: 23 - 36
#
# With an device booted and accessible via ADB, usage:
# ./build.py system
#
# This script will modify init.rc to install Magisk to the system
# partition.
# This is useful for setting up the device for testing modules,
# and developing root apps using some system writeable Android device.
#
# This only covers the "core" features of Magisk. For testing
# magiskinit, please checkout host_patch.sh.
#
#####################################################################

if [ ! -f /system/build.prop ]; then
  # Running on PC
  echo "Please run \`./build.py system\` instead of directly executing the script!"
  exit 1
fi
...
# Patch init.rc
INITRC_DIR="/system/etc/init/hw"
if [ ! -d $INITRC_DIR ]; then
  INITRC_DIR="/"
fi
echo "Patching init.rc in $INITRC_DIR"
if [ -f $INITRC_DIR/init.rc.bak ]; then
  rm -f $INITRC_DIR/init.rc
  mv $INITRC_DIR/init.rc.bak $INITRC_DIR/init.rc
  rm -f $INITRC_DIR/init.rc.bak
fi

cp -af $INITRC_DIR/init.rc $INITRC_DIR/init.rc.bak
tee -a $INITRC_DIR/init.rc <<EOF >/dev/null || exit 1
